...
#+begin_src emacs-lisp :results silent
  (setq org-clock "~/.emacs.d/lisp/org-clock-export/test.org")
...
3. Any sexp use the following variables, which are bound based values from each clock-line: start-year, start-month, start-day, start-dow, start-hour, start-minute, end-year, end-month, end-day, end-dow, end-hour, end-minute, total-hours, total-minutes
4. In addition to using those variables, the sexp be any other code that will be run at each heading containing a clock line
...
02/28/2021,1,00,Talking about mindless drivel with customer,325
#+end_example
You can use =org-clock-export-org-ql-query= to restrict to certain tags, dates, times, etc., and otherwise harness the full power of =org-ql=. 
...
| org-clock-export-data             | See above                                                                    | =( "date" (concat start-month "/" start-day "/" start-year "hours" total-hours "minutes" total-minutes "description" (org-entry-get (point) "ITEM") "hourly rate" (or (org-entry-get (point) "HOURLY-RATE") "325"))= |
|                                   |                                                                              |                                                                                                                                                                                                                  |
